...
class PreLnSelfAttentionLayer(nn.Module):
    def __init__(
        ...
        self.mha = torch.nn.MultiheadAttention(
            embedding_dim, num_heads, dropout=dropout_mha, batch_first=True
        )
    ...
    def forward(self, x, mask, initial_embedding):
        ...
        if self.enable_ctx_manager:
            with sdpa_kernel(self.attn_params[self.attention_type]):
                mha_out = self.mha(
                    q, x, x, need_weights=False, key_padding_mask=key_padding_mask
                )[0]

                if self.save_attention:
                    att_mat = self.mha(
                        q, x, x, need_weights=True, key_padding_mask=key_padding_mask
                    )[1]
        ...
        # path for ONNX export
            ...
            mha_out = self.mha(q, x, x, need_weights=False, key_padding_mask=key_padding_mask)[
                0
            ]
    ...
    def forward(self, elems, x, orig_value):
        if self.mode == "direct":
            ...
            elemtype_mask = torch.cat(
                [elems[..., 0:1] == elemtype for elemtype in self.elemtypes], axis=-1
            )
...
class MLPF(nn.Module):
    def __init__(
        ...
        if self.num_convs != 0:
            if self.input_encoding == "joint":
                ...
                for ielem in range(len(self.elemtypes_nonzero)):
                    self.nn0_id.append(
                        ffn(self.input_dim, embedding_dim, width, self.act, dropout_ff)
                    )
        ...
        self.nn_pt = RegressionOutput(
            pt_mode, embed_dim, width, self.act, dropout_ff, self.elemtypes_nonzero
        )
        self.nn_eta = RegressionOutput(
            eta_mode, embed_dim, width, self.act, dropout_ff, self.elemtypes_nonzero
        )
        self.nn_sin_phi = RegressionOutput(
            sin_phi_mode, embed_dim, width, self.act, dropout_ff, self.elemtypes_nonzero
        )
        self.nn_cos_phi = RegressionOutput(
            cos_phi_mode, embed_dim, width, self.act, dropout_ff, self.elemtypes_nonzero
        )
        self.nn_energy = RegressionOutput(
            energy_mode, embed_dim, width, self.act, dropout_ff, self.elemtypes_nonzero
        )

        if (
            self.use_pre_layernorm
        ):  # add final norm after last attention block as per https://arxiv.org/abs/2002.04745
    ...
    def forward(self, X_features, mask):
        ...
        if self.num_convs != 0:
            if self.input_encoding == "joint":
                ...
                embedding_reg = torch.stack(
                    [nn0(Xfeat_normed) for nn0 in self.nn0_reg], axis=-1
                )
                elemtype_mask = torch.cat(
                    [X_features[..., 0:1] == elemtype for elemtype in self.elemtypes_nonzero],
                    axis=-1,
        ...
        # ensure created particle has positive mass^2 by computing energy from pt and adding
        # a positive-only correction
        ...
        preds_momentum = torch.cat(
            [preds_pt, preds_eta, preds_sin_phi, preds_cos_phi, preds_energy], axis=-1
        )
