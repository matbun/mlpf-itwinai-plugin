...
def unpack_target(y, model):
    ret = {}
    ret["cls_id"] = y[..., 0].long()
    ret["charge"] = torch.clamp(
        (y[..., 1] + 1).to(dtype=torch.float32), 0, 2
    )  # -1, 0, 1 -> 0, 1, 2
...
def unpack_predictions(preds):
    ...
    ret["cls_id"][ret["cls_id"] == 1] = torch.argmax(ret["cls_id_onehot"], axis=-1)[
        ret["cls_id"] == 1
    ]
...
def load_lr_schedule(lr_schedule, checkpoint):
    ...
    if "lr_schedule_state_dict" in checkpoint["extra_state"].keys():
        ...
        raise KeyError(
            "Couldn't find LR schedule state dict in checkpoint. "
            "extra_state contains: {}".format(checkpoint["extra_state"].keys())
        )
        ...
        if not parameter.requires_grad:
            table = pd.concat(
                [
                    table,
                    pd.DataFrame(
                        {column_names[0]: name, column_names[1]: 0, column_names[2]: params},
                        index=[ii],
                    ),
                ]
            )
            ...
            table = pd.concat(
                [
                    table,
                    pd.DataFrame(
                        {column_names[0]: name, column_names[1]: params, column_names[2]: 0},
                        index=[ii],
                    ),
                ]
            )
