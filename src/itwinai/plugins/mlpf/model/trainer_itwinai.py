...
class MLPFTrainer(ItwinaiTorchTrainer):
        ...
        for split in ["train", "valid"]:  # build train, valid dataset and dataloaders
            ...
            for type_ in config[f"{split}_dataset"][config["dataset"]]:
                ...
                for sample in config[f"{split}_dataset"][config["dataset"]][type_]["samples"]:
                    version = config[f"{split}_dataset"][config["dataset"]][type_]["samples"][
                        sample
                    ]["version"]
                    split_configs = config[f"{split}_dataset"][config["dataset"]][type_][
                        "samples"
                    ][sample]["splits"]
    ...
    def train_epoch(self):
        ...
        for loss_name in epoch_loss:
            tot_losses[loss_name] = self.strategy.gather(epoch_loss[loss_name], dst_rank=0)
    ...
    def validation_epoch(self):
        ...
        for ival, batch in progress_bar:
            ...
            if self.config.save_attention and self.strategy.is_main_worker and ival == 0:
        ...
        if self.strategy.global_rank() == 0:
            ...
            for loss_name in epoch_loss:
                tot_epoch_loss[loss_name] = (
                    (torch.sum(torch.stack(tot_losses[loss_name])) / tot_steps).cpu().item()
                )
            return tot_epoch_loss
    ...
    def validation_plots(self, batch, ypred_raw):
        ...
        try:
            for attn in sorted(list(glob.glob(f"{self.config.outdir}/attn_conv_*.npz"))):
