...
class PFDataset:
    ...
    def __init__(self, data_dir, name, split, num_samples=None, sort=False):
        ...
        try:
            ...
            _logger.error(
                "Could not find dataset {} in {}, please check that you have downloaded the "
                "correct version of the dataset".format(name, data_dir)
            )
            sys.exit(1)
...
class Collater:
    def __init__(self, per_particle_keys_to_get, per_event_keys_to_get, **kwargs):
        ...
        # these quantities are a variable-length tensor per each event
        self.per_particle_keys_to_get = per_particle_keys_to_get
