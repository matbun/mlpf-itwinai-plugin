...
def pairwise_l2_dist(A, B):
    ...
    # note that this matrix multiplication can go out of range for float16 in case the
    # absolute values of A and B are large
    D = torch.sqrt(
        torch.clip(na - 2 * torch.matmul(A, torch.transpose(B, -1, -2)) + nb, 1e-6, 1e6)
    )
    return D
...
def split_msk_and_msg(bins_split, cmul, x_msg, x_node, msk, n_bins, bin_size):
    bins_split_2 = torch.reshape(
        bins_split, (bins_split.shape[0], bins_split.shape[1] * bins_split.shape[2])
    )
    ...
    x_msg_binned = torch.reshape(
        x_msg_binned, (cmul.shape[0], n_bins, bin_size, x_msg_binned.shape[-1])
    )
...
def reverse_lsh(bins_split, points_binned_enc):
    ...
    points_binned_enc_flat = torch.reshape(
        points_binned_enc, (batch_dim, n_points, n_features)
    )
    ...
    for ibatch in range(batch_dim):
        ...
        # torch._assert(torch.max(bins_split_flat[ibatch]) < n_points,
        # "reverse_lsh n_points max")
...
class MessageBuildingLayerLSH(nn.Module):
    ...
    def forward(self, x_msg, x_node, msk, training=False):
        ...
        if n_points % self.bin_size != 0:
            raise Exception(
                "Number of elements per event must be exactly divisible by the bin size"
            )
        ...
        bins_split = split_indices_to_bins_batch(
            cmul, n_bins, self.bin_size, msk, self.stable_sort
        )
...
class CombinedGraphLayer(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.dist_activation = getattr(
            torch.nn.functional, kwargs.pop("dist_activation", "elu")
        )
        ...
        for iconv in range(self.num_node_messages):
            self.message_passing_layers.append(
                GHConvDense(
                    output_dim=self.inout_dim, hidden_dim=self.inout_dim, activation="elu"
                )
            )
